...
#pragma region Create the Vertices
	Vertex v[24];
...
#pragma endregion
#pragma region Create the Indices
	UINT i[36];
...
#pragma region Create the Vertices
	...
	for ( UINT i = 1; i <= stackCount - 1; ++i )
	{
		...
		for ( UINT j = 0; j <= sliceCount; ++j )
		{
			...
			//v.TexC.x = asinf(v.Normal.x) / XM_PI + 0.5f;
			//v.TexC.y = asinf(v.Normal.y) / XM_PI + 0.5f;

			v.TexC.x = (float)j / (float)sliceCount;
			v.TexC.y = (float)i / (float)stackCount;
...
void GeometryGenerator::CreateGeosphere(float radius, UINT numSubdivisions, MeshData &meshData)
{
	...
	meshData.Vertices.clear();
	meshData.Indices.clear();
#pragma region Create an icosahedron and subdivide it
	...
	for ( UINT i = 0; i < numSubdivisions; ++i )
		Subdivide(meshData);
#pragma endregion
#pragma region Project vertices onto sphere and scale
...
void GeometryGenerator::CreateGrid(float width, float depth, UINT rowCount, UINT columnCount, MeshData &meshData)
{
	UINT vertexCount = rowCount*columnCount;
	UINT faceCount = (rowCount - 1)*(columnCount - 1) * 2;

#pragma region Create the Vertices
	float halfWidth = 0.5f*width;
	float halfDepth = 0.5f*depth;

	float dx = width / (columnCount - 1);
	float dz = depth / (rowCount - 1);

	float du = 1.0f / (columnCount - 1);
	float dv = 1.0f / (rowCount - 1);
	...
	for ( UINT i = 0; i < rowCount; ++i )
	{
		...
		for ( UINT j = 0; j < columnCount; ++j )
		{
			...
			meshData.Vertices[i*columnCount + j].Position = XMFLOAT3(x, 0.0f, z);
			meshData.Vertices[i*columnCount + j].Normal   = XMFLOAT3(0.0f, 1.0f, 0.0f);
			meshData.Vertices[i*columnCount + j].TangentU = XMFLOAT3(1.0f, 0.0f, 0.0f);

			// Stretch texture over grid.
			meshData.Vertices[i*columnCount + j].TexC.x = j*du;
			meshData.Vertices[i*columnCount + j].TexC.y = i*dv;
		}
	}
#pragma endregion
...
#pragma region Create the Indices
	...
	{
		for ( UINT j = 0; j < columnCount - 1; ++j )
		{
			meshData.Indices[k] = i*columnCount + j;
			meshData.Indices[k + 1] = i*columnCount + j + 1;
			meshData.Indices[k + 2] = (i + 1)*columnCount + j;

			meshData.Indices[k + 3] = (i + 1)*columnCount + j;
			meshData.Indices[k + 4] = i*columnCount + j + 1;
			meshData.Indices[k + 5] = (i + 1)*columnCount + j + 1;
...
void GeometryGenerator::Subdivide(MeshData &meshData)
{
	...
	for ( UINT i = 0; i < numTris; ++i )
	{
		...
		//
		// Generate the midpoints position
		//
